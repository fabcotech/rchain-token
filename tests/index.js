...
const main = async () => {
  ...
  balances2.push(await getBalance(PUBLIC_KEY_2));
  console.log(`✓ 05 purchase 1 token from bag "0", new bag ID: ${newBagId}`);
  console.log(
    '  05 dust cost: ' +
  ...
  balances2.push(await getBalance(PUBLIC_KEY_2));
  console.log(`✓ 07 update data associated with bag ${newBagId}`);
  console.log(
    '  07 dust cost: ' +
      (balances2[balances2.length - 2] - balances2[balances2.length - 1])
  );
  ...
  await sendTokens(
    data.registryUri.replace('rho:id:', ''),
    allData.bags['0'].nonce,
    PRIVATE_KEY,
    PUBLIC_KEY,
    newBagId2
  );
  balances1.push(await getBalance(PUBLIC_KEY));
  console.log(`✓ 09 send 1 token from bag "0" to new bag ID ${newBagId2}`);
  console.log(
    '  09 dust cost: ' +
      (balances1[balances1.length - 2] - balances1[balances1.length - 1])
  );
