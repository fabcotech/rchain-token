...
const creditAndSwap = require('../ft/test_creditAndSwap').main;
const deploy = require('../ft/test_deploy').main;
const updatePursePrice = require('../ft/test_updatePursePrice.js').main;

const checkPursesInBox = require('../nft/checkPursesInBox.js').main;
const createPurses = require('../nft/test_createPurses.js').main;

const { PRIVATE_KEY, PUBLIC_KEY, PRIVATE_KEY_2, PUBLIC_KEY_2 } = require('../ft/keys');
...
const main = async () => {
  ...
  console.log('tests:parallelcredits total time : ' + Math.round((new Date().getTime() - time) / 600) / 100 + 'min')
};
