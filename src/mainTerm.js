    ...
    for (@currentBags <<- bags) {
      match currentBags.get(*payload.get("bagId")) {
        ...
        bag => {
          match bag.get("quantity") - *payload.get("quantity") >= 0 {
            true => {
              new justVerifySignatureReturnCh, bagIdCh in {
                justVerifySignatureCh!((
                  bag.get("publicKey"),
                  *signature,
                  *payload,
    ...
    match *action.get("type") {
      ...
      "CREATE_TOKENS" => {
        match *action.get("payload") {
          ...
          _ => {
            return!("error: invalid payload, structure should be { 'newNonce': String, 'newBagId': Nil or String, 'bagNonce': String, 'quantity': Int, 'n': String, 'price': Nil or Int, 'publicKey': String, 'data': Any }")
          }
        }
      }
      // Anyone capabilities
      "PURCHASE_TOKENS" => {
        match *action.get("payload") {
          { "quantity": Int, "bagId": String, "newBagId": Nil \\/ String, "publicKey": String, "nonce": String, "data": _, "purseRevAddr": _, "purseAuthKey": _ } => {
            purchaseCh!(*action.get("payload"), *return)
          }
          _ => {
            return!("error: invalid payload, structure should be { 'quantity': Int, 'bagId': String, 'newBagId': Nil or String, 'publicKey': String, 'nonce': String, 'data': Any, 'purseRevAddr': String, 'purseAuthKey': AuthKey }")
          }
        }
      }
      "SEND_TOKENS" => {
        match *action.get("payload") {
          { "quantity": Int, "bagId": String, "newBagId": Nil \\/ String, "publicKey": String, "bagNonce": String, "bagNonce2": String, "data": _, } => {
            sendCh!(
              *action.get("payload"),
              *action.get("signature"),
              *return
          ...
          _ => {
            return!("error: invalid payload, structure should be { 'quantity': Int, 'bagId': String, 'newBagId': Nil or String, 'publicKey': String, 'bagNonce': String, 'bagNonce2': String, 'data': Any }")
          }
        }
      }
      _ => {
