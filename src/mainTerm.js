...
new MakeNode, ByteArrayToNybbleList,
    TreeHashMapSetter, TreeHashMapGetter, TreeHashMapContains, TreeHashMapUpdater, HowManyPrefixes, NybbleListForI, keccak256Hash(\`rho:crypto:keccak256Hash\`),
          ...
          for (@depth <<- @(map, "depth")) {
            for (@howManyPrefixes <- howManyPrefixesCh ) {
              contract iterateOnPrefixesCh() = {
                new itCh, allMapCh, TreeHashMapGetterValues in {
                  // do not move it up, the goal is reduce the number of serializatin / dezerialization
                  contract TreeHashMapGetterValues(@map, @nybList, @n, @len, @i) = {
                    // Look up the value of the node at (map, nybList.slice(0, n + 1))
                    for (@val <<- @[(map, nybList.slice(0, n)), *storeToken]) {
                      if (n == len) {
                        if (val == Nil) {
                        ...
                        if ((val / powers.nth(nybList.nth(n))) % 2 == 0) {
                          ...
                          TreeHashMapGetterValues!(map, nybList, n + 1, len, i)
                        }
                      }
                    }
                  } |
