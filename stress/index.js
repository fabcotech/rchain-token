...
const PURSES_TO_CREATE_INITIAL = 200;
...
const main = async () => {
  ...
  const createPursesBatch = async (j) => {
    ...
    balances1.push(await getBalance(PUBLIC_KEY));
    let s = '';
    s += `✓ 03 create ${PURSES_TO_CREATE} purses\n`;
    s +=
      '  03 dust cost: ' +
      (balances1[balances1.length - 2] - balances1[balances1.length - 1]) +
      '\n';
  ...
  const filename = `./stress_logs_${time}.txt`;

  for (let j = 0; j < 1000000; j += 1) {
    if (j === 0) {
      fs.writeFileSync(
        filename,
        `PURSES_TO_CREATE: ${PURSES_TO_CREATE}\PURSES_TO_CREATE_INITIAL: ${PURSES_TO_CREATE_INITIAL}\NEW_BOX_EACH_TIME: ${NEW_BOX_EACH_TIME}\n`,
        'utf8'
      );
    }
